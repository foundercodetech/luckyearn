...
import 'package:luckyearn/view/account/History/AvaitorBethistory.dart';
...
class _BetHistoryState extends State<BetHistory> with SingleTickerProviderStateMixin {
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    textWidget(text:  itemsTRX[index].status=="0"?"Pending":itemsTRX[index].status=="1"?"Win":"Loss",
                                        fontSize: width*0.05,
                                        fontWeight: FontWeight.w600,
                                        color: itemsTRX[index].status=="0"?Colors.orange:itemsTRX[index].status=="1"?Colors.green:Colors.red
  ...
  Future<void> BettingHistoryTRX() async {
    ...
    if (response.statusCode==200) {
      final List<dynamic> responseData = json.decode(response.body)['data'];
      setState(() {
        itemsTRX = responseData.map((itemsTRX) => BettingHistoryModelTRX.fromJson(itemsTRX)).toList();
      });
